...
require_once('../phpmailer/src/PHPMailer.php');
    class UserC
    {
        ...
        public function passwordForgot($email)
        {
            ...
            if($res)
            {
                $sql = "UPDATE user set enabled= :enabled, token= :token WHERE email= :email";
                $sql2=$this->db->prepare($sql);
                $token = $this->generateToken();
                $url = 'http://localhost:8080/pidev/views/reset.php?mail='.$email.'&token='.$token;
